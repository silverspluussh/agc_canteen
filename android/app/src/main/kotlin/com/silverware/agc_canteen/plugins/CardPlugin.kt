...
class CardPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    /**
     * IC card power-on reset via CAR D command with zero-length APDU.
     * Command: 1B 23 23 43 41 52 44 00
     * Returns ATR bytes (skipping the 0x00 status prefix).
     */
    ...
    /**
     * PSAM APDU exchange.
     * Command: 1B 23 23 50 53 41 4D slot LEN APDU
     * (9 + apdu.size bytes, single-byte length)
     * Skips single-byte error responses (0x01=no reset, 0x02=apdu error).
     */
    ...
    /**
     * PSAM close / power-off.
     * Command: 1B 23 23 50 4D 44 4E slot
     * (8 bytes, no length field)
     */
    ...
    /**
     * Poll for magnetic card swipe data.
     * Flushes input twice before polling (matching PdaX1-1 behaviour).
     * Polls up to 50 times, 200ms between reads.
     */
